...
const AddReview = () => {
    ...
    const fetchBusinessData = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/businesses/get/${location.state.Id}`);
            var unFilterData = response.data.Review.filter(value => value.Status === "Incomplete");
            var filterData = response.data.Review.filter(value => value.Status === "Approved");
    ...
    const handleFileUpload = (event) => {
        ...
        reader.onload = (e) => {
            ...
            const formattedArray = columnData.map((item) => ({ Description: item, Status: "Incomplete" }));
    ...
    const submitBulkData = async () => {
        try {
            ...
            const response = await axios.post(`http://localhost:5000/businesses/${location.state.Id}/reviews/bulk-upload`, {
                reviews: jsonData,
            });
    ...
    return (
        <div className="p-4 md:p-8">
            <div className="max-w-2xl mx-auto">
                ...
                <div>
                    <h3 className="text-lg font-bold mb-2">Business Id:- {businessData?.BusinessId}</h3>
                    <h3 className="text-lg font-bold mb-2">Business Name:- {businessData?.Name}</h3>
